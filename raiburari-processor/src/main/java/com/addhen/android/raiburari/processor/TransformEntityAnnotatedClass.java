  ...
  public TransformEntityAnnotatedClass(TypeElement typeElement) throws ProcessingException {
    ...
    if (typeElement.getModifiers().contains(Modifier.ABSTRACT)) {
      throw new ProcessingException(typeElement,
          ...
          typeElement.getQualifiedName().toString(), TransformEntity.class.getSimpleName());
    }
        ...
        if (e.getKind() == ElementKind.FIELD && annotation != null) {
          ...
          throw new ProcessingException(e,
              "%s is of type %s and annotated with @%s, but only Fields or setter "
                  + "Methods can be annotated with @%s", e.getSimpleName(), e.getKind().toString(),
              Transform.class.getSimpleName(), Transform.class.getSimpleName());
        }
      }
        ...
        if ((superClassPackage != null && originPackage == null) || (superClassPackage == null
          ...
          throw new ProcessingException(e,
              "The field %s in class %s can not be accessed from it transform class because of "
                  ...
                  + "as %s or make the field %s public", field.getFieldName(),
              field.getQualifiedSurroundingClassName(), mTypeElement.getQualifiedName().toString(),
              field.getQualifiedSurroundingClassName(), field.getFieldName(),
              Transform.class.getSimpleName());
        }
      }

      TransformAnnotatedField existingTransformAnnotatedField =
          transformAnnotatedElementsMap.get(field.getFieldName());
      if (existingTransformAnnotatedField != null) {
        throw new ProcessingException(e,
            "The field %s in class %s is annotated with @%s with transform name = \"%s\" "
                + "but this transform name is already used by %s in class %s", field.getFieldName(),
            field.getQualifiedSurroundingClassName(), Transform.class.getSimpleName(),
            field.getFieldName(), existingTransformAnnotatedField.getElementName(),
